...
titel = "Versuch 0 Aufgabe 1 (c)"
yName = "$U_{pp}/U_{0pp}$"
xName = "$\omega$"
yEinheit = "$dB$"
xEinheit = "$kHz$"
#pt.beschriften(titel = titel, xName = xName, yName = yName, xEinheit = xEinheit, yEinheit = yEinheit)
omega = [6.5,6.7,7.0,7.05,7.1,7.15,7.2,7.22,7.24,7.26,7.31,7.61,8,10,50,100]
Upp = [2.85,2.81,2.75,2.75,2.74,2.74,2.73,2.72,2.71,2.70,2.70,2.65,2.6,2.25,0.59,0.31]
U0pp = 2
y = [-20*np.log10(element/U0pp) for element in Upp]
x = omega
dy = [0.2 for r in x]
dx = [0.3 for r in x]
pt.zeichne(x,y,dx,dy,titel,xName,yName,xEinheit,yEinheit,xBeginn = min(x), yBeginn = min(y), safe = True)
plt.show()
